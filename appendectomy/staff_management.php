...
IF($_GET[ACT]=="LOGIN")
{
	...
	IF(MYSQL_NUM_ROWS($RESULT=MYSQL_QUERY($QUERY_STRING))==1)
	{
		...
		ECHO "<FORM ACTION=processing_form.php?ACT=GO METHOD=POST>
			<INPUT TYPE=HIDDEN NAME=NICKNAME VALUE='".$_POST[NICKNAME]."'>
			<INPUT TYPE=HIDDEN NAME=PASSWORD VALUE='".$_POST[PASSWORD]."'>
			<INPUT TYPE=SUBMIT VALUE='進入文書處理頁面' STYLE='FONT-SIZE:16;WIDTH:150'>
			</FORM>";
		ECHO "<FORM ACTION=processing_system.php?ACT=CPWD METHOD=POST>
			<INPUT TYPE=HIDDEN NAME=NICKNAME VALUE='".$_POST[NICKNAME]."'>
			<INPUT TYPE=HIDDEN NAME=PASSWORD VALUE='".$_POST[PASSWORD]."'>
			<INPUT TYPE=SUBMIT VALUE='進入密碼修改頁面' STYLE='FONT-SIZE:16;WIDTH:150'>
			</FORM>";
		
		IF($DATA[level]=='admin'||$DATA[level]=='reg_admin')
		{
			...
			ECHO "<FORM ACTION=processing_system.php?ACT=STFMGR METHOD=POST>
				<INPUT TYPE=HIDDEN NAME=NICKNAME VALUE='".$_POST[NICKNAME]."'>
				<INPUT TYPE=HIDDEN NAME=PASSWORD VALUE='".$_POST[PASSWORD]."'>
				<INPUT TYPE=SUBMIT VALUE='管理工作人員頁面' STYLE='FONT-SIZE:16;WIDTH:150'>
				</FORM>";
		}
		ELSE
		{
		...
		ECHO "<INPUT TYPE=BUTTON ONCLICK=location.href='processing_system.php' VALUE='登出' STYLE='FONT-SIZE:16;WIDTH:150'><BR>";
	}
	ELSE
	{
		ECHO "<BR>登入失敗<BR><BR>
...
ELSE IF($_GET[ACT]=="STFMGR")
{
	...
	IF($NO_OF_DATA>0)
	{
		...
		FOR($SEED=0;$SEED<$NO_OF_DATA;$SEED++)
		{
			...
			IF(strtoupper($_POST[NICKNAME])!=strtoupper($DATA[nickname]))
			{
				ECHO "<INPUT TYPE=BUTTON ONCLICK=location.href='processing_system.php?ACT=DELSTF&SID=".$DATA[staff_id]."' VALUE='刪除'>";
			}
			ECHO "</TD></TR>";
		}
		ECHO "<FORM ACTION=processing_system.php?ACT=LOGIN METHOD=POST><TR><TD COLSPAN=6 STYLE='TEXT-ALIGN:CENTER;BACKGROUND-COLOR:#EEEEFF'>
				<HR>
				<INPUT TYPE=HIDDEN NAME=NICKNAME VALUE='".$_POST[NICKNAME]."'>
				<INPUT TYPE=HIDDEN NAME=PASSWORD VALUE='".$_POST[PASSWORD]."'>
				<INPUT TYPE=SUBMIT VALUE='回上一頁' STYLE='FONT-SIZE:16'>
				</TD></TR></FORM>";
	}
	ECHO "</TABLE>";
	ECHO "</TD></TR>";
...
ELSE IF($_GET[ACT]=="DELSTF"||$_GET[ACT]=="MODSTF"||$_GET[ACT]=="ADDSTF")
{
	...
	IF($_GET[ACT]=="ADDSTF")
	{
		...
		<TABLE BORDER=0 STYLE='BACKGROUND-COLOR:#EEEEEE;WIDTH:100%'>
			...
			<TR><TD STYLE='WIDTH:150;TEXT-ALIGN:CENTER;BACKGROUND-COLOR:#DDDDDD'>等　　級</TD><TD><INPUT TYPE=RADIO NAME=LEVEL VALUE='normal' CHECKED>一般<INPUT TYPE=RADIO NAME=LEVEL VALUE='admin'>管理員</TD></TR>
	...
	ELSE
	{
		...
		IF(MYSQL_NUM_ROWS($RESULT=MYSQL_QUERY($QUERY_STRING))==1)
		{
			...
			ELSE IF($_GET[ACT]=="MODSTF")
			{
				...
				IF($DATA[level]=='admin')
					ECHO "<TR><TD STYLE='WIDTH:150;TEXT-ALIGN:CENTER;BACKGROUND-COLOR:#DDDDDD'>等　　級</TD><TD><INPUT TYPE=RADIO NAME=LEVEL VALUE='normal'>一般<INPUT TYPE=RADIO NAME=LEVEL VALUE='admin' CHECKED>管理員</TD></TR>";
				ELSE
					ECHO "<TR><TD STYLE='WIDTH:150;TEXT-ALIGN:CENTER;BACKGROUND-COLOR:#DDDDDD'>等　　級</TD><TD><INPUT TYPE=RADIO NAME=LEVEL VALUE='normal' CHECKED>一般<INPUT TYPE=RADIO NAME=LEVEL VALUE='admin'>管理員</TD></TR>";
...
ELSE IF($_GET[ACT]=="CPWD")
{
	...
	<INPUT TYPE=HIDDEN NAME=NICKNAME VALUE='".$_POST[NICKNAME]."'></TD></TR>
	<INPUT TYPE=HIDDEN NAME=PASSWORD VALUE='".$_POST[PASSWORD]."'></TD></TR>
...
ELSE IF($_GET[ACT]=='CPWDEXE')
{
	...
	ELSE IF($_POST[PASSWORD_O]!=$_POST[PASSWORD_N])
	{
		...
		{
			$QUERY_STRING="UPDATE STAFF_INFO SET PASSWORD=PASSWORD('".$_POST[PASSWORD_N]."') WHERE NICKNAME='".$_POST[NICKNAME]."' AND PASSWORD=PASSWORD('".$_POST[PASSWORD_O]."')";
			IF(MYSQL_QUERY($QUERY_STRING))
			{
				ECHO "<BR>密碼變更成功！<BR><BR>";
				ECHO "<FORM ACTION=processing_system.php?ACT=LOGIN METHOD=POST>
					<INPUT TYPE=HIDDEN NAME=NICKNAME VALUE='".$_POST[NICKNAME]."'>
					<INPUT TYPE=HIDDEN NAME=PASSWORD VALUE='".$_POST[PASSWORD_N]."'>
