...
function returnEmailList($DOCTYPE,$STATUS,$DAY_INTERVAL)
{
	...
	//NO_NOTIFY 欄位已加入時請改用：
//	$QUERY_STRING="SELECT * FROM ".$DOCTYPE." AS DOC,USER_BASIC AS UB WHERE DOC.USER_ID=UB.USER_ID AND CURRENT_STATUS='".$STATUS."' AND NO_NOTIFY=0 AND CREATED_TIME<=date_sub(NOW(), interval '".$DAY_INTERVAL."' day) AND NOTIFIED_TIME<=date_sub(NOW(), interval '".$DAY_INTERVAL."' day)";
